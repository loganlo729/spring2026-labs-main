...
def generate_character(description: str) -> CharacterSheet:
    ...
    response = ollama.chat(
        model=MODEL,
        messages=[
            {"role": "system", "content": "You are a helpful assistant that creates D&D character sheets based on a description."},
            {"role": "user", "content": f"Create a character based on this description: {description}"}
        ],
...
def generate_monster(concept: str) -> MonsterStats:
    ...
    response = ollama.chat(
        model=MODEL,
        messages=[
            {"role": "system", "content": "You are a helpful assistant that creates D&D monsters based on a concept."},
            {"role": "user", "content": f"Create a monster based on this concept: {concept}"}
        ],
...
def generate_encounter(party_level: int, num_monsters: int, theme: str) -> Encounter:
    ...
    response = ollama.chat(
        model=MODEL,
        messages=[
            {"role": "system", "content": "You are a helpful assistant that creates D&D encounters based on party level, number of monsters, and a theme."},
            {"role": "user", "content": f"Create an encounter for a party of level {party_level} with {num_monsters} monsters, following this theme: {theme}"}
        ],
